...
async function checkAndIncrementAIMessageCount(
  ...
  // Identify privileged user (mark)
  const isMark = identifier === "mark";
  ...
  if (isAuthenticated && authToken) {
    ...
    // If the request is from mark **and** the token is valid, bypass rate limits entirely
    if (isMark) {
      return { allowed: true, count, limit };
    }
  }

  // If the user *claims* to be mark but is **not** authenticated, deny the request outright
  if (isMark) {
    return {
      allowed: false,
      count: 0,
